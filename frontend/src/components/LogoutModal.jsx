    ...
    <>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-opacity-50 backdrop-blur-xs">
          <div className="relative bg-white rounded-lg shadow-lg dark:bg-gray-500 w-full max-w-md">
            {/* Close Button */}
            <button
              onClick={() => setIsModalOpen(false)}
              className="absolute top-3 right-2.5 text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer"
            >
              <svg
                className="w-5 h-5"
                fill="none"
